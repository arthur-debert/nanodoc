...
from nanodoc.v1.files import (
    ...
    run_mixed_content_bundle,
    run_traditional_bundle,
)
...
def test_run_mixed_content_bundle(tmpdir):
    # Create test files
    test_file1 = tmpdir.join("test_file1.txt")
    ...
    result = run_mixed_content_bundle(lines)
...
def test_run_traditional_bundle(tmpdir):
    # Create test files
    test_file1 = tmpdir.join("test_file1.txt")
    test_file1.write("Test content 1")
    test_file2 = tmpdir.join("test_file2.txt")
    ...
    result = run_traditional_bundle(lines)
...
def test_inline_file_inclusion(tmpdir):
    ...
    assert (
        f"Line 3 with multiple references: {expected_inline1} and {expected_inline2}."
    ) in result

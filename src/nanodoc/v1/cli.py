...
# directly to the main command instead of to the 'run' subcommand
@click.group(
...
@click.pass_context
def run(ctx, v, n, toc, no_header, sequence, style, txt_ext, theme, use_v2, sources):
    ...
    try:
        ...
        logger.info(f"Run command called with theme: {theme}")
        ...
        extensions = list(TXT_EXTENSIONS)
        if txt_ext:
            for ext in txt_ext:

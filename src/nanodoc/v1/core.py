...
def run(
    ...
    """Process source files and generate documentation using v1 implementation.
    ...
    Args:
        ...
        theme (str): Theme to use for output (not used in v1).
    ...
    """
    # Get content items from sources
    content_items = get_files_from_args(list(sources), txt_ext)

    # Process the files using the v1 implementation
    return run_all(
...
def run_all(
    ...
    for file_index, (_, items) in enumerate(file_groups.items()):
        ...
        for item in items:
            ...
            file_output, num_lines = run_file(
                item,
                line_number_mode,
                line_counter,
                show_header,

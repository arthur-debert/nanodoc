...
def run_mixed_content_bundle(lines):
    """Process a mixed content bundle (text and file paths).
    ...
    for line in lines:
        ...
        if is_file_path_line(line):
            try:
                ...
                # If the file doesn't exist, treat it as regular text
                result.append(line)
        else:
            result.append(line)
    return "\n".join(result)
...
def run_traditional_bundle(lines):
...
def expand_bundles(bundle_file):
    ...
    if is_mixed_content_bundle(lines):
        ...
        return run_mixed_content_bundle(lines)
    else:
        # Process as traditional bundle (list of files)
        return run_traditional_bundle(lines)
